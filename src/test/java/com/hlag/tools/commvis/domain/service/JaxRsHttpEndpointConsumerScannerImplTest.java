...
class JaxRsHttpEndpointConsumerScannerImplTest {
    ...
    void shouldExtractAllEndpointInformation_whenScanClasspath() {
        ...
        Mockito.verify(endpointFactory, Mockito.times(1)).createHttpConsumer(Mockito.eq("test.http.jaxrs.ConsumerEndpoints"), Mockito.eq("receivesAPostRequest"), Mockito.eq("POST"), Mockito.eq("endpoint/a"));
    }
}